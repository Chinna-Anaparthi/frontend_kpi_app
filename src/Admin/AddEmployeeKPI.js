                  ...
                  {row.subcategories.map((subcategory, subcategoryIndex) => (
                    <div key={subcategoryIndex}>
                      ...
                      {subcategory.subcategory && (
                        <div>
                          {subcategory.metrics.map((metric, metricIndex) => (
                            <div key={metricIndex} style={{ display: 'flex', alignItems: 'center', marginBottom: 10 }}>
                              <FormControl variant="outlined" sx={{ minWidth: 120, marginRight: 2 }}>
                                <InputLabel id={`metric-label-${index}-${subcategoryIndex}-${metricIndex}`}>Metric</InputLabel>
                                <Select
                                  labelId={`metric-label-${index}-${subcategoryIndex}-${metricIndex}`}
                                  name="metric"
                                  value={metric.metric}
                                  onChange={(e) => handleMetricChange(e, index, subcategoryIndex, metricIndex)}
                                  input={<OutlinedInput label="Metric" />}
                                >
                                  {renderMetricOptions(index, subcategoryIndex)}
                                </Select>
                              </FormControl>
                              <FormControl variant="outlined" sx={{ minWidth: 120 }}>
                                <InputLabel id={`quantity-target-label-${index}-${subcategoryIndex}-${metricIndex}`}>Quantity Target</InputLabel>
                                <Select
                                  labelId={`quantity-target-label-${index}-${subcategoryIndex}-${metricIndex}`}
                                  name="quantityTarget"
                              ...
                              <Tooltip title="Add Metric">
                                {metricIndex === subcategory.metrics.length - 1 && (
                                  <IconButton aria-label="add metric" onClick={() => handleAddMetric(index, subcategoryIndex)}>
                                    <AddIcon />
                                  </IconButton>
                                )}
                              </Tooltip>
                              <Tooltip title="Remove Metric">
                                {metricIndex !== 0 && (
                                  <IconButton
                                    ...
                                    onClick={() => handleRemoveMetric(index, subcategoryIndex, metricIndex)}
                                  >
                                    <RemoveIcon />
                                  </IconButton>
                                )}
                      ...
                      {subcategoryIndex !== 0 && (
                        <Tooltip title="Remove Subcategory">
                          <IconButton aria-label="remove subcategory" onClick={() => handleRemoveSubcategory(index, subcategoryIndex)}>
                            <RemoveIcon />
                          </IconButton>
                        </Tooltip>
                      )}
